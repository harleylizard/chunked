...
public record SetBlock(BlockPredicate predicate, BlockStateProvider block) implements Action {
    public static final MapCodec<SetBlock> CODEC = RecordCodecBuilder.mapCodec(builder -> builder.group(BlockPredicate.CODEC.fieldOf("predicate").forGetter(SetBlock::predicate), BlockStateProvider.CODEC.fieldOf("block").forGetter(SetBlock::block)).apply(builder, SetBlock::new));
    ...
    public void run(ServerLevel level, BlockPos blockPos, RandomSource random) {
        ...
        if (chunk != null && predicate.matches(level, blockPos)) {
